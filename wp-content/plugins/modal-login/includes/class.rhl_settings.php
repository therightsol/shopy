...
class rhl_settings {
	var $added_rules;
	function __construct($plugin_id='rhl'){
		$this->id = $plugin_id;
	...
	function options($t){
		...
		$t[$i]->options[]=	(object)array(
				'type' 			=> 'subtitle',
				'label'			=> __('Logout form','rhl')
		);	
		$labels = array(
			'head_logout' 			=> __('Logout form title','rhl'),
		);	
		foreach($labels as $id => $label){
			$t[$i]->options[]=(object)array(
				'id'	=> 'rhl_label_'.$id,
		...
		$t[$i]->options = array(
			...
			(object)array(
				'id'		=> 'disable_bootstrap_login',
				'label'		=> __('Disable bootstrap javascript(wp-login.php)','rhl'),
				'type'		=> 'yesno',
				'default'	=> '0',
				'description'=> __('Choose yes if other plugins or the theme is adding the bootstrap javascript on wp-login.php and you dont want the modal login one to be loaded.','rhl'),
				'el_properties'	=> array(),
				'save_option'=>true,
				'load_option'=>true
			),
