...
class modal_login_ajax {
	...
	function rp(){
		...
		//------------------------------------------------------------------------------------------------------------
		//$action = 'resetpass';
		$user = check_password_reset_key($_POST['key'], $_POST['user_login']);
		if ( is_wp_error($user) ) {
			$ret = array(
				'R'=>'ERR',
				'MSG'=>$user->get_error_message()
	...
	function form_label($id,$label,$echo=true){
		$label = apply_filters('rhl_label', $label, $id );
		if($echo)echo $label;
		return $label;
	}
	...
	function retrieve_password() {
		...
		if ( !$user_data ) {
			$errors->add('invalidcombo', $this->form_label( 'lostpassword_invalidcombo', __('Invalid username or e-mail.', 'rhl'), false) );
			return $errors;
		}
		...
		if ( ! $allow )
			return new WP_Error('no_password_reset', $this->form_label( 'no_password_reset', __('Password reset is not allowed for this user', 'rhl'), false) );
		...
		// MODIFIED and updated to reflect wp-login.php for Wordpress 4.3 adding time() to hash.
		...
		// MODIFIED retrieve_password_message to have all 4 parameters and then changed class.spl_email_filters.php accordingly.

		$message = apply_filters( 'retrieve_password_message', $message, $key, $user_login, $user_data );
		
		if ( $message && !wp_mail($user_email, $title, $message) )
			return new WP_Error('wp_email_error', $this->form_label( 'lostpassword_wp_email_error', __('Possible reason: your host may have disabled the mail() function...', 'rhl'), false) );
