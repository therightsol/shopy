...
class plugin_modal_login {
	...
	function __construct($args=array()){
		...
		//-----------
		global $wp_version;
		$this->wp43plus = version_compare( $wp_version, '4.3', '>=' );		
		//-----------
	...
	function init(){
		...
		if(is_admin()){
			...
			if('1'==$rhl_plugin->get_option('disable_bootstrap_login','0',true)){
				$bs = false;
			}	
		}else{
	...
	function plugins_loaded(){
		...
		if( $this->wp43plus ){
			...
			require_once RHL_PATH.'includes/class.rhl_email_filters.prewp431.php';
			require_once RHL_PATH.'includes/class.modal_login_ajax.prewp431.php';
		}
		...
		if(is_admin()){
			...
			*/
			$dc_options = array(
				...
				'tdom'			=> 'rhl',
				'module_url'	=>  RHL_URL.'options-panel/'
			);
