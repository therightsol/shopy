...
class modal_login_frontend {
	...
	function jquery_check(){
		global $wp_scripts;
		if( is_object($wp_scripts) && $wp_scripts->registered && isset($wp_scripts->registered['jquery']) && version_compare($wp_scripts->registered['jquery']->ver,'1.7.2') < 0  ){
...
<?PHP
	...
	function head(){
		...
		$login_actions = apply_filters('rhl_login_actions', array( 'postpass', 'logout', 'lostpassword', 'retrievepassword', 'resetpass', 'rp', 'register', 'login', 'maintenance' ) );
		$login_actions = is_array( $login_actions ) ? $login_actions : array() ;
		if($this->own_jquery){
?>
...
rhl_login_actions = <?php echo json_encode($login_actions)?>;
...
rhl_jQuery(document).ready(function($){
	...
	*/
	...
	<?php if($this->close_on_click_enabled()):?>
...
<?php
	...
	function close_on_click_enabled(){
		$theme = wp_get_theme();
		if( false!==strpos( strtolower($theme->get('Name')),'avada') ){
			return false;
		}
