...
echo __( 'Some commissions have not been credited properly.', 'yith_wc_product_vendors' ) . "\n\n";

echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n\n";

do_action( 'woocommerce_email_before_commission_table', $commission, $sent_to_admin, $plain_text );

echo strtoupper( sprintf( __( 'Commission number: %s', 'yith_wc_product_vendors' ), $commission->id ) ) . "\n";
echo date_i18n( __( 'jS F Y', 'yith_wc_product_vendors' ), strtotime( $commission->get_date() ) ) . "\n";
