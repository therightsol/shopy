...
function yith_wcpsc_pr_install_woocommerce_admin_notice() {
    ?>
    <div class="error">
        <p><?php _e( 'YITH Product Size Charts for WooCommerce Premium is enabled but not effective. It requires WooCommerce in order to work.', 'yit' ); ?></p>
    </div>
    <?php
}
...
if ( !defined( 'YITH_WCPSC_VERSION' ) ) {
    define( 'YITH_WCPSC_VERSION', '1.0.4' );
}
...
function yith_wcpsc_pr_init() {
    ...
    load_plugin_textdomain( 'yith-wcpsc', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
