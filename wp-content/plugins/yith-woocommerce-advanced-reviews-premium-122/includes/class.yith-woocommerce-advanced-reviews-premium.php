...
class YITH_WooCommerce_Advanced_Reviews_Premium extends YITH_WooCommerce_Advanced_Reviews {
    ...
    /**
     * @var $load_more_type Choose if a "load more" should be shown, with textual link (value 2) or button link (value
     *      3) or don't show it (value 1)
     */
    ...
    /**
     * @var $hide_inappropriate_review_threshold Number of users reporting a review as inappropriate content needed for
     *      hiding the review
     */
    ...
    protected function __construct () {
        ...
        add_filter ( 'ywar_product_reviews_submit_reviews_denied_text', array ( $this, 'ywar_product_reviews_submit_reviews_denied_text' ), 10, 2 );
    }
    ...
    public function show_expanded_review_content ( $review ) {
        ...
        $review_title             = '';
        $review_content           = '<span class="review_content">' . esc_attr ( $review->post_content ) . '</span>';
...
<a id="vote_no_' . $review->ID . '" class="' . $no_class . '" href="' . $voting_no_url . '" data-vote_review="-1" data-id_review="' . $review->ID . '" title="' . __ ( "This reviews is not helpful", 'yith-woocommerce-advanced-reviews' ) . '"></a>';
    ...
    public function enqueue_scripts () {
        ...
        wp_localize_script ( 'ywar-script', 'ywar', array (
            'nonce_value'           => $nonce,
            'loader'                => $loader,
    ...
    public function user_can_edit_review ( $review ) {
        ...
        return current_user_can ( 'manage_options' ) || ( get_current_user_id () == $review->_ywar_review_user_id );
    }
    ...
    public function edit_review_callback () {
        ...
        update_post_meta ( $review_id, $this->meta_key_rating, $review_rating );
    ...
    public function get_ajax_comments_callback () {
        ...
        if ( ( ! isset( $product_id ) ) || ( ! is_numeric ( $product_id ) ) || ( intval ( $product_id ) < 0 ) ) {
            wp_send_json ( array ( - 1, __ ( 'Product ID is in an invalid format', 'yith-woocommerce-advanced-reviews' ) ) );
        }
    ...
    public function customer_reviews_for_product ( $product_id, $review_id = 0 ) {
        ...
        $result = $wpdb->query ( $wpdb->prepare ( "select p.ID, p.post_content, p.post_title, p.post_status, p.post_parent, m2.meta_value as approved, m3.meta_value as rating
                ...
                where p.post_type = 'ywar_reviews' and p.post_author = %d
