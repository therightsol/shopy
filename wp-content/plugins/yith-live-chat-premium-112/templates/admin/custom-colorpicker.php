...
/**
 * Color Picker Plugin Admin View
 *
...
<div id="<?php echo $id ?>-container" <?php if (isset( $option['deps'] )): ?>data-field="<?php echo $id ?>" data-dep="<?php echo $this->_panel->get_id_field( $option['deps']['ids'] ) ?>" data-value="<?php echo $option['deps']['values'] ?>" <?php endif ?> class="yit_options rm_option rm_input rm_text">
    <div class="option">
        <input type="text" name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo esc_attr( $db_value ) ?>" <?php if (isset( $option['std'] )) : ?>data-default-color="<?php echo $option['std'] ?>"<?php endif ?> class="panel-colorpicker" <?php echo $custom_attributes ?> />
    </div>
