jQuery(function($){
    var type                        = $( '#yith-wcbm-badge-type').data( 'type' ),
        ...
        ad_list_configurable        = new Array( 1 ,         1 ,         1 ,         1 ,         0 ,         1 ,         1 ,         1 ,         1 ,         0),
        ...
        ad_list                     = new Array( ad_list_configurable, ad_list_bg_color_def, ad_list_txt_color_def ),
        ...
        preview_render              = function() {
            switch(type){
                ...
                case 'css':
                    ...
                    preview_badge.html('<div class="yith-wcbm-css-s1"></div><div class="yith-wcbm-css-s2"></div><div class="yith-wcbm-css-text">'+ css_text.val() +'</div>');
                    // my_timeout prevents too many ajax requests
                    window.clearTimeout(my_timeout);
                    my_timeout = setTimeout(function(){
                        $.ajax({
                            type       : "POST",
                            data       : {  id_badge_style  : css_badge.val() , 
                ...
                case 'advanced':
                    ...
                    preview_badge.html('<div class="yith-wcbm-shape1"></div><div class="yith-wcbm-shape2"></div><div class="yith-wcbm-simbol-sale">Sale</div><div class="yith-wcbm-simbol-percent">%</div><div class="yith-wcbm-simbol-off">OFF</div><div class="yith-wcbm-sale-percent">50</div><div class="yith-wcbm-simbol-save">Save</div><div class="yith-wcbm-sale-save">$ 15</div>');
                    if (first_open == 1){
                        preview_badge.hide();
                        first_open = 0;
                    }
                    ...
                    my_timeout = setTimeout(function(){
                        $.ajax({
                            type       : "POST",
                            data       : {  id_badge_style  : advanced_badge.val() , 
                ...
                default:
                    // TEXT
                    preview_badge.html(input_text.val());
                    preview_style_badge.html('');
                    preview_badge.css({
    ...
    anchor_point.on("change", function() {
        switch( anchor_point.val() ) {
            case 'top-left':
                pos_top.val(0); pos_bottom.val('auto'); pos_left.val(0); pos_right.val('auto');
                break;
            case 'top-right':
                pos_top.val(0); pos_bottom.val('auto'); pos_left.val('auto'); pos_right.val(0);
                break;
            case 'bottom-left':
                pos_top.val('auto'); pos_bottom.val(0); pos_left.val(0); pos_right.val('auto');
                break;
            case 'bottom-right':
                pos_top.val('auto'); pos_bottom.val(0); pos_left.val('auto'); pos_right.val(0);
                break;
            default:
        }
        preview_render();
    ...
    input_opacity.on("input", function(){
        output_opacity.html(input_opacity.val());
    });
    ...
    */
    button_select_advanced.click(function(){
        ...
        if ( ad_list[0][ad_index] == 1 ){
            ...
            advanced_no_change_color = 0;
            advanced_message_not_conf.fadeOut('fast', function(){
                advanced_options.fadeIn('fast');
            });
        }else{
            advanced_options.fadeOut('fast', function(){
                advanced_message_not_conf.fadeIn('fast');
            });
        }
