		...
		if ( $this->is_priced_per_product() ) {
			...
			if ( $price ) {
				...
				foreach ( $this->price_index[ 'regular_price' ][ $min_or_max ] as $component_id => $product_id ) {
					...
					if ( $item_qty ) {
						$composited_product = $this->get_composited_product( $component_id, $product_id );
						$display_price      += $item_qty * $composited_product->get_regular_price( $min_or_max, true, true );
					}
				}
		...
		if ( $this->is_priced_per_product() && ! empty( $this->composite_data ) ) {
			...
			if ( $this->hide_price_html() || $this->min_composite_price === '' ) {
				...
				if ( $suppress_range_format ) {
					...
					if ( $this->min_composite_regular_price > $this->min_composite_price || $this->max_composite_regular_price > $this->max_composite_price ) {
